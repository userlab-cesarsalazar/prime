...
const getReconciliation = (params) => {
  ...
  let query = `SELECT D.client_id,num_control,total,observations,a.status as status_conciliation FROM documents D
                 INNER JOIN account_reconciliation a on D.id = a.document_id
                 WHERE a.status = 'PENDING' ORDER BY D.id DESC`
  
  switch (params.type) {
    case 'client':
      query = `SELECT D.client_id,num_control,total,observations,a.status as status_conciliation
                FROM documents D
                INNER JOIN clientes C  on D.client_id = C.client_id
                INNER JOIN account_reconciliation a on D.id = a.document_id
