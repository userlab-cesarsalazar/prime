...
const { response, fileResponse, getBody, escapeFields } = require(`${
    isOffline ? '../..' : '.'
}/commons/utils`)
...
module.exports.readPackagesByManifest = async () => {
...
module.exports.exportManifest = async (event) => {
    ...
    try {
        ...
        let manifestoHeaders = [
            {name: 'Tracking', column: 'tracking', width: 33.50},
            ...
            {name: 'Descripcion', column: 'description', width: 51.50},
            {name: 'Valor Declarado', column: 'costo_producto', width: 16.17, numFmt: '"$"#,##0.00'}, //Missing
        ];
    ...
    new Promise((resolve, reject) => {
        ...
        if (data && data.sheets && data.sheets.length > 0) {
            data.sheets.forEach((sheet, i) => {
                ...
                newSheet.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: '053E81' } };
                ...
                if (sheet.totals && sheet.totals.length > 0) {
                    sheet.totals.forEach(t => {
                        newSheet.getCell(`${t}${sheet.data.length + 2}`).value = { formula: `SUM(${t}2:${t}${sheet.data.length + 1})` };
                        newSheet.getCell(`${t}${sheet.data.length + 2}`).font = { bold: true };
                    });
                }
